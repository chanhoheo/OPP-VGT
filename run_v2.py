...
def postprocess(cfg, img_lists, root_dir, sub_dirs, outputs_dir_root, obj_name):
    ...
    model_filted_track_path = osp.join(deep_sfm_dir, "model_filted_track")
    os.makedirs(model_filted_bbox_path, exist_ok=True)
    os.makedirs(model_filted_track_path, exist_ok=True)
    if not cfg.post_process.skip_bbox_filter:
        ...
        )  # crop 3d points by 3d box and save as colmap format. box filtering하고 sfm_ws/model_filted_bbox에 저장  이때 3d point개수는 111476
    else:
    ...
    track_length, points_count_list = filter_tkl.get_tkl( ## track_length = 10, points_count_list = list of 111476
        model_filted_bbox_path, thres=cfg.dataset.max_num_kp3d, show=False
    ) # '/sfm_ws/model_filted_bbox 모델 열음. 필터링은 하지않음
    filter_tkl.vis_tkl_filtered_pcds(
        ...
        outputs_dir,
        vis3d_pth,
    )  # visualization only 필터링하고 tkl_model에 저장됨 그러나 이후의 필터링에서 이 tkl_model이 사용되지는 않음

    xyzs, points_ids, _xyzs, _points_ids = filter_points.filter_track_length_v2(
        model_filted_bbox_path, track_length, model_filted_track_path
    )
    # crop 3d points by 3d box and track length sfm_ws/model_filted_bbox불러와서 필터링. 필터링 결과 xyzs: (15657, 3)
    # model_filted_bbox의 3dpoints[points_ids[i]].xyz == xyzs[i]
    merge_xyzs, merge_idxs = filter_points.hard_merge(xyzs, points_ids, model_filted_track_path, model_filted_track_path)  # merge 3d points by distance between points
    # merge_xyzs: (12123, 3), merge_idxs: dict len(12123), 
    '''
    filter_Track_legnth까지 뽑힌 xyzs를 thr이내의 포인트들은 합치는 식으로(날리는식) 필터링.
    즉, xyzs[merge_idxs[0]] = merge_xyzs[0] 
    merge_idxs: {new_point_id: [old_point1_id, old_point2_id, ...]} -> filted_bbox의 idx임.
                new_point_id: [0, xyzs.shape[0]]
                old_point_id*: point idx in Points3D.bin(filted_bbox)

    ({[x1, y1], [x2, y2], ...} => [mean(x_i), mean(y_i)])
    '''

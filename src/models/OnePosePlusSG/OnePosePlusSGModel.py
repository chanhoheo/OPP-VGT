...
from src.models.SuperGlue import SuperGlue

class OnePosePlusSG_model(nn.Module):
    def __init__(self, config, profiler=None, debug=False):
        ...
        # self.superglue_pretrained = self.config['superglue']['pretrained']
        # if self.superglue_pretrained is not None:
        #     logger.info(
        #         f"Load pretrained superglue from {self.superglue_pretrained}"
        #     )
        #     self.superglue.load_state_dict(torch.load(self.superglue_pretrained, "cpu"))

        #     if self.config["superglue"]["pretrained_fix"]:
    ...
    def forward(self, data):
        ...
        kpts2d = torch.tensor([[i,j] for i in range(64) for j in range(64)], device=kpts3d.device).reshape(1, -1, 2)
        if self.training:
            kpts2d = torch.cat((kpts2d, kpts2d))
        

        kpts3d_proj = self.proj3d(kpts3d.reshape(-1, 3)).reshape(data['bs'], -1, 2)

        # 2. SuperGlue
        self.superglue(data, desc3d_db, query_feat_c, kpts3d_proj, kpts2d)

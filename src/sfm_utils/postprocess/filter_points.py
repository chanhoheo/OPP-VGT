...
def filter_track_length(model_path, track_length, output_path):
    """ Filter 3d points by track length """
    points_model_path = osp.join(model_path, 'points3D.bin') # sfm_ws의 model_filtered_bbox 
    points3D = read_write_model.read_points3d_binary(points_model_path) # 111476개

    xyzs, points_idxs = filter_by_track_length(points3D, track_length) #xyzs: (15657, 3) points_idxs: (15657,) points3D에서 xyzs의 idx를 담고있음. 즉 points3D에서 선택된 얘들 idxs

    return xyzs, points_idxs


def filter_track_length_v2(model_path, track_length, output_path):
    """ Filter 3d points by track length """
    # points_model_path = osp.join(model_path, 'points3D.bin') # sfm_ws의 model_filtered_bbox 
    # points3D = read_write_model.read_points3d_binary(points_model_path) # 111476개
    from src.utils.colmap.read_write_model import read_model, write_model
    ...
    #pop
    new_points3d = {idx: points3D[k] for idx,k in enumerate(points_idxs)}
    write_model(cameras, images, new_points3d, output_path, '.bin')

    merge_xyzs = xyzs
    merge_idxs = {i: np.array([points_idxs[i]]) for i in range(len(xyzs))}

    return xyzs, points_idxs, merge_xyzs, merge_idxs

def hard_merge(xyzs, points_idxs, model_path, output_path, dist_threshold=1e-3):
    """ 
    Merge points which are close to others. ({[x1, y1], [x2, y2], ...} => [(x_1), (y_1)])
    """
    from scipy.spatial.distance import pdist, squareform
    from src.utils.colmap.read_write_model import read_model, write_model
    cameras, images, points3D = read_model(model_path, ext='.bin')
    points3D_key = np.array(list(points3D.keys()))
    ...
    for j in range(distance_matrix.shape[0]):
        ...
        if np.isin(points_idxs[idxs], points3D_idx_record).any():
            continue
        white_list += list(points3D_key[idxs][:1])

        points = xyzs[idxs][0] # new point
        ret_points = np.append(ret_points, points.reshape(1, 3), axis=0)
        ret_idxs[ret_points_count] = points_idxs[idxs][:1]
        ret_points_count += 1
        
        points3D_idx_record = points3D_idx_record + points_idxs[idxs].tolist()
    
    new_points3d = {idx: points3D[k] for idx,k in enumerate(white_list)}

...
class OnePosePlusVDataset(Dataset):
    def __init__(
        self,
        anno_file,
        pad=True,
    ...
    # new read_anno3d for OnePosePlusV
    # 1. use keypoints3d as pcd
    # 2. mask visible 3d points. (optionally sample the 3d points)
    # 3. return visible 3d points' feats, keypoints, etc.
    # 4. we don't need to return assignmatrix
    
    # To do this, we need following data
    # 1. ply path
    # 2. GT pose (Intrisic is not required)
    def read_anno3d_filter_invisible(
        self,
        avg_anno3d_file,
        normal_vector_file,
        gt_pose,
        ...
        # filter invisible 
        # load normal vector
        # normal_vector = torch.Tensor(np.loadtxt(normal_vector_file))
        # gt_R = gt_pose[:3, :3]
        # gt_T = gt_pose[:3, 3]
        # # calculate dot prod beteween cam and points
        # cam_position = ((-gt_R.t() @ gt_T).reshape(1, 3)).float()
        # point_to_cam_vec = cam_position - keypoints3d
        # dot_prod = torch.einsum("ij, ij -> i", normal_vector, point_to_cam_vec)
        # # filter
        # mask = (dot_prod>=0).reshape(-1)
        



        # keypoints3d_v = keypoints3d[mask]
        # avg_descriptors3d_v = avg_descriptors3d[:, mask]
        # avg_scores_v = avg_scores[mask]



        # mask3d keypoints3d should be applied
        # if assignmatrix is not None:
        #     assignmatrix_mask = (mask[assignmatrix[1].long()] == True)
        #     assignmatrix_v = assignmatrix[:, assignmatrix_mask]
        #     new_assignmatrix = []
        #     for i in range(assignmatrix_v.shape[1]):
        #         new_assignmatrix.append(torch.where((keypoints3d_v[:, 0] == keypoints3d[assignmatrix_v[1][i].long()][0]) & (keypoints3d_v[:, 1] == keypoints3d[assignmatrix_v[1][i].long()][1]) & (keypoints3d_v[:, 2] == keypoints3d[assignmatrix_v[1][i].long()][2]))[0][0])
        #     new_assignmatrix = torch.Tensor(new_assignmatrix).reshape(-1)
        #     assignmatrix_v[1] = new_assignmatrix
        ...
        # original keypoints3d should be applied
        if pad:
            if self.split == "train":
                if assignmatrix is not None:
                    (
        ...
        # visible_7000 = []
        # for i in range(keypoints3d_v.shape[0]):
        #     tmp = torch.where((keypoints3d[:, 0]==keypoints3d_v[i][0]) & (keypoints3d[:, 1] == keypoints3d_v[i][1]) & (keypoints3d[:, 2] == keypoints3d_v[i][2]))[0]
        #     if tmp.sum() != 0:
        #         visible_7000.append(tmp[0])
        # visible_7000 = torch.Tensor(visible_7000).long()

        normal_vector = torch.Tensor(np.loadtxt(normal_vector_file))
        normal_vector = normal_vector[padding_index]
        gt_R = gt_pose[:3, :3]
        gt_T = gt_pose[:3, 3]
        # calculate dot prod beteween cam and points
        cam_position = ((-gt_R.t() @ gt_T).reshape(1, 3)).float()
        ...
        assignmatrix = assignmatrix[:, (mask[assignmatrix[1,:]] == True)]
    ...
    def build_assignmatrix(
        ...
        if pad:
            conf_matrix = torch.zeros(
                self.shape3d, self.n_query_coarse_grid, dtype=torch.float
    ...
    def read_anno(self, img_id, image_warp_adapt=False):
        ...
        tkl_model_path = osp.join(anno['avg_anno3d_file'].rsplit('/', 2)[0], 'sfm_ws', 'model_filted_track')
        ply_file = None
        normal_vector_file = None

        # ply file path
        # for file in os.listdir(tkl_model_path):
        #     if file.endswith('.ply'):
        #         ply_file = osp.join(tkl_model_path, file)
        # assert ply_file is not None

        if self.split == "train":
            normal_vector_file = osp.join(tkl_model_path, 'normal_vector.txt')
            assert os.path.exists(normal_vector_file)
            ...
            (
                ...
                assign_matrix,
                # visible_7000,
            ) = self.read_anno3d_filter_invisible(
                avg_anno3d_file,
                normal_vector_file,
                data['query_pose_gt'],
                pad=self.pad,
        ...
        if self.split == "train":
            ...
            # mkpts_proj = mkpts_proj[visible_7000]
            ...
            #     mkpts_proj = mkpts_proj[~out_of_boundry_mask]
            #     # assign_matrix = assign_matrix[:, ~out_of_boundry_mask]
            #     visible_7000 = visible_7000[~out_of_boundry_mask]
            #     K_crop_warpped = (
            #         torch.from_numpy(homo_sampled).to(torch.float32) @ K_crop
            #     )  # FIXME: incorrect! Shouldn't H \times K directly
            #     data.update(
            ...
            # mkpts_proj_rounded = mkpts_proj_rounded[~invalid]
            # mkpts_proj = mkpts_proj[~invalid]
            # visible_7000 = visible_7000[~invalid]

            # unique, index = np.unique(mkpts_proj_rounded, return_index=True, axis=0)

            # mkpts_proj_rounded = mkpts_proj_rounded[index]
            # mkpts_proj = mkpts_proj[index]
            # visible_7000 = visible_7000[index]



            # # make conf and fine matrix
            # conf_matrix = torch.zeros(
            ...
            # )
            # mkpts_proj_rescaled = (
            #     mkpts_proj_rescaled.round()
            # )

            # j_ids = (
            #     mkpts_proj_rescaled[:, 1] * self.w_c
            #     +mkpts_proj_rescaled[:, 0]
            # )
            # j_ids = j_ids.long()
            ...
            # keypoints2D_fine_selected = mkpts_proj[~invalid_mask]

            # conf_matrix[visible_7000, j_ids] = 1
            # fine_location_matrix[visible_7000, j_ids] = keypoints2D_fine_selected 
